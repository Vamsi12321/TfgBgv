...
import {
  ...
  TrendingUp, Target, ArrowRight, CalendarCheck, X, Building2,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 p-4 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">All organizations • Hiring pipeline analytics</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "Total Jobs", value: summary.totalJobs || 0, icon: Briefcase, color: "from-blue-500 to-indigo-600", tc: "text-blue-700" },
            { label: "Open Positions", value: summary.openPositions || 0, icon: Target, color: "from-emerald-500 to-teal-600", tc: "text-emerald-700" },
            { label: "Total Applicants", value: summary.totalApplicants || 0, icon: Users, color: "from-amber-500 to-orange-600", tc: "text-amber-700" },
            { label: "Total Hired", value: summary.totalHired || 0, icon: CheckCircle2, color: "from-green-500 to-emerald-600", tc: "text-green-700" },
          ].map((s, i) => (
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-400">
                          <span className="font-medium text-indigo-500">{job.orgName}</span> • {job.department} • {job.location} • {totalCandidates} candidates
                        </p>
                      </div>
                    </div>
                                ...
                                <div key={round} className="bg-white rounded-xl p-4 border border-gray-100 hover:border-blue-100 hover:shadow-sm transition">
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {Object.entries(statuses).map(([status, count]) => (
                                      count > 0 && (
                                        <span key={status} className={`text-[10px] font-bold px-2 py-1 rounded-lg ${ROUND_STATUS_COLORS[status]}`}>
                                          {count} {status}
                                        </span>
                                      )
                                    ))}
