...
export default function SuperAdminReportsPage() {
  ...
  const fetchCandidates = async (orgId, forceRefresh = false) => {
    ...
    // Only fetch if we don't have data or org changed (unless forced refresh)
    if (!forceRefresh && candidates.length > 0 && reportsFilters.organizationId === orgId) {
      setLoading(false);
      return;
    }
          ...
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={() => {
                  if (selectedOrg) {
                    fetchCandidates(selectedOrg, true); // Force refresh
                  }
                }}
