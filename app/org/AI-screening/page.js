...
export default function OrgAIResumeScreeningPage() {
  ...
  const [prefillOpen, setPrefillOpen] = useState({}); // { [filename]: true/false }
  const [prefillForms, setPrefillForms] = useState({}); // { [filename]: { firstName, lastName, email, phone } }
  const [addedCandidates, setAddedCandidates] = useState({}); // { [filename]: "added" | "loading" | "error" }
